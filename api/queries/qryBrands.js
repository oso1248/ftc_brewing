...
function getDetailByNameCsxPre(name) {
  return db('brnd_brw AS brw')
    ...
    .select('brw.brand', 'pre.tk_chp', 'pre.tk_uni', 'pre.lines', 'pre.cooler', 'pre.seps', 'pre.acp', 'pre.tk_sch', 'pre.tk_fill', 'pre.note', 'brw.id')
...
function getDetailByNameCsxPost(name) {
  return db('brnd_brw AS brw')
    ...
    .select('brw.brand', 'post.tk_chp', 'post.tk_uni', 'post.lines', 'post.seps', 'post.tk_sch', 'post.note', 'brw.id')
    .where({ 'brw.brand': name })
    .first();
}
function getDetailByNameFilPre(name) {
  return db('brnd_fin AS fin')
    ...
    .select('fin.brand', 'pre.tk_sch', 'pre.lines', 'pre.tk_trp', 'pre.tk_fbt', 'pre.tk_fill', 'pre.inj', 'pre.ctrl', 'pre.note', 'fin.id')
    .where({ 'fin.brand': name })
    .first();
}
function getDetailByNameFilPost(name) {
  return db('brnd_fin AS fin')
    ...
    .select('fin.brand', 'post.tk_sch', 'post.lines', 'post.tk_trp', 'post.tk_fbt', 'post.recover', 'post.note', 'fin.id')
    .where({ 'fin.brand': name })
    .first();
}
function getDetailByNameRelPre(name) {
  return db('brnd_fin AS fin')
    ...
    .select('fin.brand', 'pre.tk_fbt', 'pre.lines', 'pre.tk_lin', 'pre.tk_dft', 'pre.recover', 'pre.ctrl', 'pre.note', 'fin.id')
    .where({ 'fin.brand': name })
    .first();
}
function getDetailByNameRelPost(name) {
  return db('brnd_fin AS fin')
    ...
    .select('fin.brand', 'post.tk_fbt', 'post.lines', 'post.tk_lin', 'post.tk_dft', 'post.recover', 'post.note', 'fin.id')
    .where({ 'fin.brand': name })
    .first();
}
...
function patchCsxPre(changes) {
  ...
  return db.transaction((trx) => {
    ...
    changes.forEach((data) => {
      const query = db('csx_pre').where('brw_id', data.id_brnd).update(data.db, data.method).transacting(trx);
      queries.push(query);
    });
...
function patchCsxPost(changes) {
  ...
  return db.transaction((trx) => {
    let queries = [];
    changes.forEach((data) => {
      const query = db('csx_post').where('brw_id', data.id_brnd).update(data.db, data.method).transacting(trx);
      queries.push(query);
    });
    Promise.all(queries).then(trx.commit).catch(trx.rollback);
  });
...
function patchFltrPre(changes) {
  ...
  return db.transaction((trx) => {
    let queries = [];
    changes.forEach((data) => {
      const query = db('fltr_pre').where('fin_id', data.id_brnd).update(data.db, data.method).transacting(trx);
      queries.push(query);
    });
    Promise.all(queries).then(trx.commit).catch(trx.rollback);
  });
...
function patchFltrPost(changes) {
  ...
  return db.transaction((trx) => {
    let queries = [];
    changes.forEach((data) => {
      const query = db('fltr_post').where('fin_id', data.id_brnd).update(data.db, data.method).transacting(trx);
      queries.push(query);
    });
    Promise.all(queries).then(trx.commit).catch(trx.rollback);
  });
...
function patchRelPre(changes) {
  ...
  return db.transaction((trx) => {
    let queries = [];
    changes.forEach((data) => {
      const query = db('rel_pre').where('fin_id', data.id_brnd).update(data.db, data.method).transacting(trx);
      queries.push(query);
    });
    Promise.all(queries).then(trx.commit).catch(trx.rollback);
  });
...
function getRecipeByNameChp(name) {
  return db('brnd_brw AS brw')
    ...
    .select('brw.brand', 'chp.param_d', 'chp.param_p', 'chp.param_aa', 'chp.param_abw', 'chp.param_rdf', 'chp.note', 'brw.id')
    .where({ 'brw.brand': name })
    .first();
}
function getRecipeByNameSch(name) {
  return db('brnd_brw AS brw').join('sch_params AS sch', 'sch.brw_id', '=', 'brw.id').select('brw.brand', 'sch.cc', 'sch.acp', 'sch.note', 'brw.id').where({ 'brw.brand': name }).first();
}
...
function patchRecipeChp(changes) {
  ...
  return db.transaction((trx) => {
    let queries = [];
    changes.forEach((data) => {
      const query = db('chp_params').where('brw_id', data.id_brnd).update(data.db, data.method).transacting(trx);
      queries.push(query);
    });
    Promise.all(queries).then(trx.commit).catch(trx.rollback);
  });
...
function patchRecipeSch(changes) {
  ...
  return db.transaction((trx) => {
    let queries = [];
    changes.forEach((data) => {
      const query = db('sch_params').where('brw_id', data.id_brnd).update(data.db, data.method).transacting(trx);
      queries.push(query);
    });
    Promise.all(queries).then(trx.commit).catch(trx.rollback);
  });
...
function patchRecipeFin(changes) {
  ...
  return db.transaction((trx) => {
    let queries = [];
    changes.forEach((data) => {
      const query = db('fin_params').where('fin_id', data.id_brnd).update(data.db, data.method).transacting(trx);
      queries.push(query);
    });
    Promise.all(queries).then(trx.commit).catch(trx.rollback);
  });
