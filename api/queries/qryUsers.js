...
async function add(data) {
  const [{ id }] = await db('brewery')
    .select('id')
    .where('brewery', data.brewery_id);
  data.brewery_id = id;
...
function getAll() {
  return db('users as use')
    .select('use.username', 'use.email', 'use.permissions', 'brw.brewery')
    .join('brewery as brw', 'brw.id', '=', 'use.brewery_id')
...
async function change(name, changes) {
  const [{ id }] = await db('brewery')
    .select('id')
    .where('brewery', changes.brewery_id);
