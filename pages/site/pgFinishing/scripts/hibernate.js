...
async function sendUpdate(ev) {
  ...
  if (fails.length === 0) {
    axios
      .patch('/api/hibernate//hibernated/tank/' + id, data)
      .then((data) => {
...
function selectTank() {
  ...
  let options = document.getElementsByName('int_vessel')[0].options;
  let id = options[index].id;
  axios.post('/api/hibernate/hibernated/tank/get/' + id).then((data) => {
    let res = data.data[0];
...
document.getElementById('download-xlsx').addEventListener('click', () => {
  hibernateTable.download('xlsx', 'hibernation_log.xlsx', {
    sheetName: 'hibernate',
  });
});
