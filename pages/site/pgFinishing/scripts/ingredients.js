...
function createListType(api, parent, title, type) {
  axios
    .post(api, { active: true, type: `${type}` })
    .then((res) => {
      let list = res.data;
      list.forEach((elem) => {
        let listItem = elem[title];
...
function loadIngredients() {
  ...
  axios
    .post('/api/brand/fin/ingredient/brand/get/', { brand: brand })
    .then((data) => {
      let res = data.data;
...
async function submit() {
  ...
  axios
    .post('/api/inventory/fin/injection/log/add', data)
    .then((data) => {
      alert('Added');
      softReset();
    })
...
function invFinInjectDatesWeek() {
  ...
  axios
    .post('/api/inventory/fin/injection/log/dates/weekly')
    .then((data) => {
...
function invFinInjectDatesMonth() {
  ...
  invDates.innerHTML = `<option value="" disabled selected hidden>Select Date</option>`;
  axios
    .post('/api/inventory/fin/injection/log/dates/monthly')
    .then((data) => {
      let invDate = data.data;
      return invDate.map((listItem) => {
        let invDate = createNode('option');
