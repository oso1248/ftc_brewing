...
async function sendWad(ev) {
  ...
  if (fails.length === 0) {
    axios
      .post('/api/inventory/process/wad/add', data)
      .then((res) => {
        let data = res.data;
...
async function sendLoss(ev) {
  ...
  if (fails.length === 0) {
    axios
      .post('/api/inventory/process/loss/add', data)
      .then((res) => {
        let data = res.data;
...
function wadAdditionTable() {
  axios
    .post('/api/inventory/process/wad/get')
    .then((res) => {
...
function transAdditionTable() {
  axios
    .post('/api/inventory/process/trans/get')
    .then((res) => {
      let tableData = res.data;
      tableData.map((elem) => {
        elem.created_at = DateTime.fromISO(elem.created_at).toFormat('yyyy-MM-dd HH:mm');
...
function lossAdditionTable() {
  axios
    .post('/api/inventory/process/loss/get')
    .then((res) => {
      let tableData = res.data;
      tableData.map((elem) => {
        elem.created_at = DateTime.fromISO(elem.created_at).toFormat('yyyy-MM-dd HH:mm');
